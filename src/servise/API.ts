...
const getPostsOfUser=async()=>{
    return await axiosInstance.get('/posts').then (response=> console.log( response.data))
}
const getCommentsOfPost=async()=>{
    return await axiosInstance.get('/comments').then (response=> console.log( response.data))
}
